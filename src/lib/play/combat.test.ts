...
describe('fightMonster', () => {
	...
	it('Soul Weaver ≥3 also grants simultaneous attack through corruption', () => {
		const p = makePlayer({
			barrier: 1,
			attackDice: dice('arcane', 5),
			spirits: [spirit(1, { 'Soul Weaver': 3 })]
		});
...
describe('Blood Hunter (inCombat)', () => {
	it('deals +1 damage per Broken Barrier, capped at 4', () => {
		// Broken barrier = maxTokens − barrier ⇒ 10 − 4 = 6.
		const p = makePlayer({
			barrier: 4,
			maxBarrier: 10,
			attackDice: [],
	...
	it('scales below the cap', () => {
		// Broken barrier = maxTokens − barrier ⇒ 6 − 4 = 2.
		const p = makePlayer({
			barrier: 4,
			maxBarrier: 6,
			attackDice: [],
