...
describe('new EffectAction kinds (via applyTrigger)', () => {
	...
	it('restoreBrokenBarrier restores broken barrier (clamped at full health)', () => {
		withSynthClass('SynthPurify', [
			{
				trigger: 'onRest',
				breakpoints: [{ count: 1, actions: [{ kind: 'restoreBrokenBarrier', amount: 5 }] }]
			}
		]);
		// Start with 2 broken barrier (maxTokens 6 − barrier 4); restore ≥2 ⇒ 0.
